        ...
        cJSON_ArrayForEach(plugin_item, plugins_array) {
            ...
            if (cJSON_IsArray(implements)) {
                ...
                cJSON_ArrayForEach(impl_item, implements) {
                    if (impl_count >= MAX_IMPLEMENTS) break;
...
int test_cjson()
{
    ...
    for (int i = 0; i < config.available_apis_count; i++) {
        printf("  [%d] Name: %s\n      Plugin: %s\n", 
               i, 
               config.available_apis[i].name, 
               config.available_apis[i].plugin);
    }
    ...
    for (int i = 0; i < config.plugins_count; i++) {
        ...
        printf("      Implements: ");
        for (int j = 0; j < config.plugins[i].implements_count; j++) {
            printf("%s%s", config.plugins[i].implements[j], 
                   (j < config.plugins[i].implements_count - 1) ? ", " : "");
        }
        printf("\n");
    }
